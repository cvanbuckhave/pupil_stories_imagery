    ...
    for p, trial, sdm in ops.split(dm_.subject_nr, dm_.trialid):
        ...
        if count_nonnan(pupil) > 0:
            ...
            if sdm.subtype.unique[0] != 'dynamic':
                # Pupil-size mean
                mean = reduce(pupil, operation=np.nanmedian)
...
# Plot traces
def plot_traces(dm, plot_type='non-dynamic', bl=False, slopes=True):
...
def plot_correlations(dm, what='all'):
    ...
    if what == 'by-subtype':
        ...
        fig, axes = plt.subplots(1,3, figsize=(42, 14), sharey=False) 
        ...
        plt.ylabel('Mean questionnaire scores');plt.xlabel('Pupil-size mean differences (a.u.)')
        plt.ylim([0.9, 6]);plt.yticks(np.arange(1, 6, 1))
        ...
        plt.ylabel('Mean questionnaire scores');plt.xlabel('Pupil-size slope differences (a.u.)')
        plt.ylim([0.9, 6]);plt.yticks(np.arange(1, 6, 1))
...
def main_plots(dm_to_plot, which):
